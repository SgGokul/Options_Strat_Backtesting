...
## function to run backtests in multiprocessing way for each chunk of entry times
...
if __name__ == '__main__':  
    ## to prevent multiprocessing error
    freeze_support()  
    ...
    df = pd.concat({key: df.set_index('Stats') for key, df in results.items()}, axis=1)
    df = df.T
